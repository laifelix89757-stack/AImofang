...
export const DesignStudio: React.FC<DesignStudioProps> = ({ moduleConfig }) => {
  ...
  useEffect(() => {
    const checkAvailability = async () => {
      ...
      // We don't need to decrypt it here, just check if it exists in storage
      const globalKey = localStorage.getItem(securityService.STORAGE_KEY) || localStorage.getItem('nova_global_api_key');
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (msg.includes("403") || msg.includes("PERMISSION_DENIED") || msg.includes("API key not valid")) {
         // If we are in managed mode (Global Key exists), it means the Admin's key is bad.
         const globalKey = localStorage.getItem(securityService.STORAGE_KEY) || localStorage.getItem('nova_global_api_key');
         if (globalKey) {
           setError("管理员配置的 API Key 无效或额度不足 (403)。请联系管理员。");
         } else {
    ...
    <div className="flex flex-col h-full gap-4">
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full overflow-hidden">
        ...
        <div className="w-full lg:w-1/3 flex flex-col gap-4 overflow-y-auto pr-2 pb-20 custom-scrollbar">
          ...
          {error && (
            <div className="p-3 bg-red-900/50 border border-red-700 text-red-200 text-xs rounded-lg break-words">
              {error}
            </div>
          )}
        </div>
             ...
             <div className="text-center p-8">
                {loading ? (
                  ...
                  <div className="text-gray-600 flex flex-col items-center">
                    ...
                    <p>等待任务提交</p>
                    {isManagedMode && <p className="text-xs text-green-500 mt-2">企业托管模式已激活</p>}
                  </div>
                )}
             </div>
           )}
